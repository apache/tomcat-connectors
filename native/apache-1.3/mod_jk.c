                                       ...
                                       unsigned num_of_log_items)
{
    ...
    for (h = 0; h < num_of_log_items; h++) {
        if (log_names[h] && log_values[h]) {
            ap_table_set(r->notes, log_names[h], log_values[h]);
        }
    }
}
                           ...
                           jk_ws_service_t *s, jk_server_conf_t * conf)
{
    ...
    stateless = ap_table_get(r->subprocess_env, JK_ENV_STATELESS);
    if (stateless) {
        if (*stateless == '\0') {
...
static const command_rec jk_cmds[] = {
    ...
    {"JkLocalAddrIndicator", jk_set_local_addr_indicator, NULL, RSRC_CONF, TAKE1,
    ...
    {"JkSSLPROTOCOLIndicator", jk_set_ssl_protocol_indicator, NULL, RSRC_CONF, TAKE1,
